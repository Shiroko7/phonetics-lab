...
assert.equal(check.rubrics.humanCategories.heavyAccent.slots, 1)
assert.equal(check.rubrics.humanCategories.heavyAccent.flagged, 1)
assert.equal(check.rubrics.majorityAccentOrError.precision, 1)
const divided = structuredClone(gold)
divided[0].words[0].phoneAnnotations = ['(K) AE1 T', '(K) AE1 T', '{K} AE1 T', '{K} AE1 T', 'K AE1 T']
assert.equal(evaluatePhoneFlags(divided, raw).rubrics.humanCategories.noCategoryMajority.slots, 1)
assert.equal(evaluatePhoneFlags(gold.map(r => ({ ...r, split: 'calibration' })), raw, 80, { split: 'calibration' }).split, 'calibration')
assert.throws(() => evaluatePhoneFlags(gold, raw, 80, { split: 'final' }), /locked/)
