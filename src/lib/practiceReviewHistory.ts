...
import { sameLine, sameScoringScale, type Attempt, type PhoneStat } from './practice.ts'
import { normalizeWord, type StruggledWord } from './struggles.ts'
...
export function reviewHistory(attempts: Attempt[], dict: Dictionary, preferences: PracticePreferences, manual: StruggledWord[] = []) {
  ...
  const compatible = latest ? ordered.filter((a) => sameScoringScale(a, latest)) : []
  ...
  for (const attempt of compatible) {
    ...
    for (const [key, entry] of takeEntries) {
      const count = wordCounts.get(key) ?? { seen: 0, flagged: 0 }
      latestWords.set(key, entry ? { ...entry, struggleCount: count.flagged, totalAttempts: count.seen } : null)
    }
